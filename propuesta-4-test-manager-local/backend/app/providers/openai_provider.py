...
class OpenAITestCaseGenerator:
    ...
    async def generate(
        ...
        start = time.monotonic()
        try:
            # max_tokens: cuando max_cases es None, subimos el techo a 4096 para
            # darle al modelo espacio cuando decide generar 6-12 casos por
            # cobertura amplia (1 por CA + edge cases). Cuando max_cases viene
            # explícito, escala lineal con cap duro de 8192.
            base_tokens = 2048 if max_cases is None else 1024
            max_tokens = min(8192, base_tokens + (max_cases or 8) * 320)
            # temperature 0.4: balance entre exploración (cobertura) y consistencia
            # de formato. Con 0.2 el modelo se pegaba al patrón "3 casos" del
            # prompt anterior; con 0.4 explora más combinaciones de dominios y
            # vectores de borde. INVEST y extractor siguen en 0.1 (determinismo).
            response = await _client().beta.chat.completions.parse(
                model=_MODEL,
                messages=_build_messages(
                    _SYSTEM_QA,
    ...
    async def generate_with_context(
        ...
        *,
        ...
        Mantiene la misma operación (`tc_generate_single`) en logs/cuota: es
        otra forma de generar casos para una HU, no una operación nueva con
        pricing distinto.
        ...
        start = time.monotonic()
        try:
            # Bump menor del cap de tokens cuando hay contexto (más material
            # → más casos esperados). Sigue acotado a 8192 (techo del modelo).
    ...
    async def generate_batch(
        ...
        try:
            ...
            response = await _client().beta.chat.completions.parse(
                model=_MODEL,
                messages=_build_messages(
                    _SYSTEM_QA, _batch_tc_user_prompt(stories_json, max_cases), _MODEL,
                ),
