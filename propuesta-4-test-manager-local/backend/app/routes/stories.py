...
async def analyze_invest(
    ...
    if not story:
        raise HTTPException(status_code=404, detail="Historia no encontrada")
    try:
        story = await invest_svc.analyze_and_save(story, current_user)
    except QuotaExceeded as e:
        raise _quota_to_http(e)
    return _enrich(story, story_repo)
...
async def generate_batch(
    ...
    try:
        return await tc_svc.generate_batch(stories, current_user, max_cases=data.max_cases)
    except QuotaExceeded as e:
        raise _quota_to_http(e)
    except ValueError as e:
...
async def generate_test_cases(
    ...
    try:
        story = await tc_svc.generate_for_story(story, current_user, max_cases=max_cases)
    except QuotaExceeded as e:
        raise _quota_to_http(e)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
