...
import {
  ...
  ShieldCheck, DollarSign, Sparkles,
} from 'lucide-react'
import clsx from 'clsx'
import { useAuth } from '../hooks/useAuth'
import { meApi } from '../api'

// Sidebar minimalista: solo "carpetas" (proyectos) en el top level.
// Las features que viven dentro de un proyecto (casos de prueba, historias,
// métricas, test plans) se navegan desde la pantalla del proyecto o desde
// el bloque contextual "Proyecto actual" más abajo.
const NAV_ITEMS = [
  { to: '/',  label: 'Proyectos',  icon: FolderKanban },
]
        ...
        <div className="flex items-center gap-2">
          ...
          {usage && (
            <Link
              ...
              title={
                usage.bypass
                  ? 'Sin límite (admin o cap deshabilitado)'
                  : `Llevás $${usage.cost_usd.toFixed(4)} de $${usage.budget_usd.toFixed(2)} este mes (${usage.calls} llamadas)`
              }
              className={clsx(
              ...
              {currentProjectId && (
                <>
                  ...
                  <Link
                    ...
                    title={collapsed ? 'Métricas de Calidad' : undefined}
                    className={clsx(
                      'flex items-center gap-2.5 py-2 text-sm font-medium transition-colors rounded-slds mx-1',
                      collapsed ? 'justify-center px-2' : 'px-3',
                  ...
                  <Link
                    ...
                    title={collapsed ? 'Test Plans' : undefined}
                    className={clsx(
                      'flex items-center gap-2.5 py-2 text-sm font-medium transition-colors rounded-slds mx-1',
                      collapsed ? 'justify-center px-2' : 'px-3',
                  ...
                  >
                    ...
                    {!collapsed && <span>Test Plans</span>}
