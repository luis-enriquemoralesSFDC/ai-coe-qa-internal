...
export default function ProjectChatDrawer({ projectId, activeStoryId }: Props) {
  ...
  const sendMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      if (status === 429) {
        ...
        toast.error(err?.response?.data?.detail ?? 'Mensaje inválido')
      } else if (status === 502) {
        toast.error('El asistente no respondió. Intenta de nuevo.')
      } else {
  ...
  function handleClear() {
    if (!window.confirm('¿Borrar todo el historial del chat de este proyecto?')) {
      return
    }
        ...
        >
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-slds-neutral-7">
                  {activeStoryId
                    ? `Con contexto de HU#${activeStoryId}`
            ...
            {!messagesQuery.isLoading && isEmpty && (
              <div className="text-center text-sm text-slds-neutral-7 py-8">
                ...
                <div className="text-xs mt-1 px-4">
                  Solo conozco lo que está en este proyecto y, si estás dentro
                  de una HU, también su detalle. No puedo ver otros proyectos
                  ni datos del sistema.
                </div>
              </div>
            )}
            {messages.map((m) => (
          ...
          >
            <textarea
              ...
              placeholder={
                activeStoryId
                  ? 'Pregunta sobre esta HU o el proyecto…'
